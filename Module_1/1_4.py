...
def get_data(website_url, inputt):
    ...
    with open('output_data.txt', 'w') as file:
            for lst in data_list:
                ...
                file.write(f" country name {lst[-1]}  ")

                file.write(f"  Total cases {lst[-2]}      ")
                file.write( f" Active cases: {lst[-8]}   ")
                if(inputt==country):
                    print(str(country_data[country]))
                file.write( f" Total deaths: {lst[-4]}    ")
                file.write(  f" Total recovered:{lst[-6]}  ")
                file.write(  f" Total tests: {lst[-12]}      ")
                if(inputt==country):
                    print("")
                file.write(  f"Deaths/million:{lst[-11]}  ")  
                file.write(  f"Tests/million:{lst[-10]}   ")
                file.write(  f"New case:{lst[-3]} ")
                if(inputt==country):
                    print("")
                file.write(  f"New death:{lst[-5]} ")
                file.write( f"New recovered:{lst[-7]} \n")
    
    
                # file.write(str(country_data[country]))
    print("check output_data.txt for all country data ")
